...
/// <summary>
/// Filters items based on modification date.
/// </summary>
public class ModificationDateFilter : FileSystemFilterBase, IFilterMetadata
{
    private readonly DateTime? _startDate;
    private readonly DateTime? _endDate;
    
    public ModificationDateFilter(DateTime? startDate, DateTime? endDate)
    {
    ...
    {
        if (_startDate.HasValue && info.LastWriteTime < _startDate.Value)
            return false;
            
        if (_endDate.HasValue && info.LastWriteTime > _endDate.Value)
            return false;
    ...
    public override string Description
    {
        get
        {
            ...
            if (_startDate.HasValue)
                return $"Modified: after {_startDate.Value:yyyy-MM-dd}";
            if (_endDate.HasValue)
                return $"Modified: before {_endDate.Value:yyyy-MM-dd}";
            return "Modified: Any time";
        }
    }
    
    // IFilterMetadata implementation
    public static string FilterName => "Modification Date Filter";
    
    public static string FilterDescription => "Filter by modification date";
